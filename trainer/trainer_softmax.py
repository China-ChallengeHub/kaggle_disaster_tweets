# -*- coding: utf-8 -*-
# @Time    : 2021/1/22 12:37 下午
# @Author  : jeffery
# @FileName: trainer_softmax.py
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.data_loader):
            ...
            if self.config.config['trainer']['is_adversarial_training'] and self.config.config['adversarial_training']['type']=='FGM': # 对抗训练
                adv_train.attack()
                adv_preds = self.model(input_ids,attention_masks)
    ...
    def _inference(self):
        ...
        acc = module_mertric.categorical_accuracy(ps,ls)
        valid_result_file = self.test_data_loader.dataset.data_dir.parent / 'valid_result' /'{}-{}-{}-{}-{}.jsonl'.format(
        ...
        if not valid_result_file.parent.exists():
            valid_result_file.parent.mkdir()
        valid_result_writer = valid_result_file.open('w')
        ps = ps.argmax(dim=-1)
        for item_id,pred,label,text in zip(ides,ps,ls,ts):
            valid_result_writer.write(json.dumps({
                        "id": item_id,
                        "text": text,
                        "pred": int(pred),
                        "label":int(label)
