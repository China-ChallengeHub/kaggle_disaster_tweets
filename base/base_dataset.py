...
class BaseDataSet(Dataset):
    ...
    def __init__(self, transformer_model, overwrite_cache, force_download, cache_dir):
        ...
        if transformer_model:
            self.transformer_config = AutoConfig.from_pretrained(transformer_model, force_download=force_download,
                                                                 cache_dir=cache_dir)

            if 'albert' in transformer_model:
                self.tokenizer = BertTokenizer.from_pretrained(transformer_model, force_download=force_download,
                                                               cache_dir=cache_dir)
            else:
                self.tokenizer = AutoTokenizer.from_pretrained(transformer_model, force_download=force_download,
                                                               cache_dir=cache_dir)
    ...
    def save_features_to_cache(self):
        ...
        if not self.feature_cache_file.parent.exists():
            self.feature_cache_file.parent.mkdir()
